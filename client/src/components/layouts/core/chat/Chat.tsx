...
class Chat extends React.Component<ChatProps, ChatState> {
    ...
    postMessage = async () => {
        await fetch(`/api/channels/${this.props.currentChannel!.room_id}/messages`, {
            method: 'POST',
            headers: {
    ...
    render() {
        const {messages, currentChannel, imgSrc, user, name} = this.props;
        return (
            <React.Fragment>
                ...
                <div className="message-container" >
                    {
                        messages.filter(m => currentChannel && m.room_id === currentChannel.room_id).map(msg => {
                            return <Message key={msg.id || Math.floor(Math.random() * 100) + 1} imgSrc={imgSrc} messageContent={msg.body} direction={user && msg.user_id !== user.id ? "left" : "right"} date={moment(msg.created_at).format('hh:mm a')} />
                        })
                    }
